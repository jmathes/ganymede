...
2. Collect the professor's current artifacts: one real slicing prompt, one Claude grading of an Aristotle result, one "Claude handoff" exchange. These are the spec for the orchestrator's prompts.
...
8. Add spend and time guards: Claude tokens per slice, Aristotle tasks per paper, and a hard stop that notifies the user instead of looping forever.
9. Acceptance test: the professor runs it on one of his manuscripts end to end.
